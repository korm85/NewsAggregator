...
3. **Grab the still frame immediately** — a single uncompressed canvas frame, with no `await` of any kind ahead of it (in particular, no exposure lock and no settle delay first — see below for why that ordering changed). This is the strictly-gated starting frame, captured under whatever auto-exposure/WB was live at that instant, as close to zero added lag from the gate-pass moment as the browser's own camera pipeline allows. No scoring or multi-frame comparison; take it or leave it. `ImageCapture.takePhoto()` (the browser's native photo pipeline, Chrome/Android only) is **not used at all** — removed because it applies irreversible hardware tone mapping, judged worse for a color-calibration anchor than the resolution it would have gained.
4. *Only now* — after the still is already captured — `tryLockCapture()` (attempt manual exposure/WB/focus lock) and `sleep(sensorSettleMs = 500ms)` to let it settle. An earlier version ran these two steps *before* the still grab, inherited from when the still was scored from a multi-frame burst that needed settled exposure across all of it; once the still became a single anchor frame that ordering was pure added latency (the saved pixels were captured 500ms+ after the gates actually passed) with no compensating benefit, so it moved to here, ahead of the video instead, where consistency across the sweep's multiple frames still has real value for the offline glare-removal step.
...
7. Stop the video recording. Best-effort: `null` on any failure or unsupported browser, and a recording failure never affects the already-captured still.
...
10. Return `{ imageUrl, blob, metadata }`; `main.ts` maps this into a `StoredCapture` and persists it (§10). `stillSource` no longer exists as a field — there's only one still-image pipeline now.
...
- **Resolved**: the still anchor's ~500ms of avoidable capture lag (exposure lock + `sensorSettleMs` sleep running *before* the frame grab, inherited from the old multi-frame-burst design) has been fixed by reordering — the still is now grabbed with no `await` ahead of it, before the lock/settle, which moved to precede the video instead (§8). Remaining latency is the camera pipeline's own sensor-to-`<video>`-element delay and up to one `requestVideoFrameCallback` tick, neither of which is measurable from inside this codebase — **unverified on a real device**, same caveat as everything else here.
