...
3. **Grab `stillFrameCount` (3) still candidates immediately** — uncompressed canvas frames, with no `await` of any kind ahead of the first one (in particular, no exposure lock and no settle delay first — see below for why that ordering changed), `stillFrameIntervalMs` (120ms) apart. The first reflects the strictly-gated starting geometry with as close to zero added lag from the gate-pass moment as the browser's own camera pipeline allows; the 120ms spacing on the other two is a deliberate, small, known delay chosen specifically to span a typical human blink (100-400ms), not an accidental one. All three are captured under whatever auto-exposure/WB was live at that moment (no lock yet). Each is scored (sharpness minus a clipping penalty, `frameScore.ts`) and the highest-scoring one is flagged `bestStillIndex` — a default for callers that want one image, not a claim the other two are worthless; nothing is discarded. `ImageCapture.takePhoto()` (the browser's native photo pipeline, Chrome/Android only) is **not used at all** for any of them — removed because it applies irreversible hardware tone mapping, judged worse for a color-calibration anchor than the resolution it would have gained.
4. *Only now* — after all three stills are already captured — `tryLockCapture()` (attempt manual exposure/WB/focus lock) and `sleep(sensorSettleMs = 500ms)` to let it settle. An earlier version ran these two steps *before* any still grab, inherited from when the still was scored from a 15-frame burst that needed settled exposure across all of it; once the still became a small, fast set of anchor frames that ordering was pure added latency (the saved pixels were captured 500ms+ after the gates actually passed) with no compensating benefit, so it moved to here, ahead of the video instead, where consistency across the sweep's multiple frames still has real value for the offline glare-removal step.
...
10. Return `{ imageUrl, blob, metadata }`, where `blob` is the best-scored candidate and `metadata.stillCandidates` carries all three (with scores) plus `bestStillIndex`; `main.ts` maps this into a `StoredCapture` and persists it (§10). `stillSource` no longer exists as a field — `ImageCapture` is gone, so there's only one still-image pipeline (this one), just now producing several candidates instead of one.
...
- `StoredCapture` fields: `blob` (the best-scored still candidate, §8, used everywhere a single image is needed), `stillCandidates` (`Blob[]`, all candidates including `blob`), `stillScores` (`number[]`, index-aligned with `stillCandidates`), `bestStillIndex`, all pose/smile metrics (`offAxisDeg`, `offAxisVec`, `rollDeg`, `pitchDeg`, `yawDeg`, `mar`, `smileWidthRatio`, mouth-box dimensions — `mouthBoxWidth` doubles as the distance-gate ratio), `exposureLockSuccess`, `captureMode`, `capturedAt`, cardboard/card fields (`cardboardMode`, `cardMarkersDetected`, `cardAllMarkersVisible`, `cardIsFlat`), `lightDirection` (nullable), video fields (`videoBlob`/`videoMimeType`/`videoDurationMs`, all nullable — the primary artifact, §8).
...
- Lightbox (tap a thumbnail): full image (swappable), a **still-candidates strip** below it when more than one exists — every candidate from §8, the best one outlined and badged, tapping any swaps the main preview — the video player (if present, shown **full-frame**, no crop simulation — a deliberate scope cut, see the "video crop" note in §14), a formatted date/time subtitle, Save-to-device (a `download` link, always targeting `blob`/the best candidate regardless of which is being previewed) and Delete actions, and a collapsed `<details>` "Capture details" disclosure holding all raw metadata (pitch/yaw/roll, smile width, MAR, mouth-box %, exposure-lock state, capture mode, per-candidate still scores with the best starred, and — cardboard captures only — marker/flatness status and light direction). The "Image source" row (canvas vs. ImageCapture) that used to appear here was removed along with `ImageCapture` itself (§8) — there's only one still-image pipeline now, just multi-candidate. There is no UI action to promote a non-best candidate to become `blob` — reviewing an alternate is possible, adopting it as the saved image is not, short of deleting and retaking.
...
1. Is **3 candidates, 120ms apart** (§8) the right amount of still-image insurance for how the downstream system actually uses it — enough to matter for blink/blur coverage, or still thin enough (vs. the old 15-frame/~5s-window design) that it's closer to theater than a real fix? Is there value in letting a human promote a non-best candidate to the saved `blob`, which the gallery currently doesn't support?
...
4. Is **storage growth** (uncapped across sessions, video recorded at a higher bitrate, and now 3 still candidates instead of 1 per capture) a real problem at expected usage volumes, and if so what's the cheapest fix (cap total on-device size? lower the bitrate target? drop to 1-2 still candidates? compress?)
